...
import {
  MapPin, Phone, Mail, ArrowLeft, ArrowUpRight, BedDouble, Bath, Maximize,
...
import { WA, TEL, EMAIL, GOLD, Instagram, Logo, waFor } from '@/lib/brand'
...
function descripcion(p: Prop) {
  const amb = p.ambientes ? `${p.ambientes} ambientes` : (p.tipo === 'Terreno' ? 'lote' : 'amplios espacios')
...
export default function PropertyView({ p }: { p: Prop }) {
  ...
  return (
    <div id="top">
      {/* NAV */}
      <header className="sticky top-0 z-40 backdrop-blur-md" style={{ background: 'rgba(11,11,13,0.78)', borderBottom: '1px solid var(--line-soft)' }}>
        ...
        <div className="grid lg:grid-cols-[1.6fr_1fr] gap-3 mb-3">
          <button onClick={() => setLightbox(true)} className="relative h-[300px] md:h-[460px] rounded-2xl overflow-hidden group" style={{ border: '1px solid var(--line)' }}>
            ...
            <span className="absolute top-3 left-3 text-[0.66rem] font-semibold uppercase tracking-[0.14em] px-2.5 py-1 rounded-full"
              style={{ background: p.op === 'Venta' ? GOLD : 'rgba(11,11,13,0.8)', color: p.op === 'Venta' ? '#0B0B0D' : '#F4F1EA', border: p.op === 'Venta' ? 'none' : '1px solid rgba(244,241,234,0.25)' }}>
              {p.op}
            </span>
          </button>
          ...
          <aside className="lg:sticky lg:top-24">
            <div className="rounded-2xl p-6" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
              ...
              <div className="pt-5 flex items-center gap-3" style={{ borderTop: '1px solid var(--line)' }}>
                <div className="w-10 h-10 rounded-full flex items-center justify-center font-serif" style={{ background: 'var(--gold-soft)', color: GOLD }}>MV</div>
                <div>
                  <div className="text-sm font-medium">María S. Vilariño</div>
                  <div className="text-[0.74rem]" style={{ color: 'var(--fg-faint)' }}>Matrícula CSI 6726 · CPI 9801</div>
                </div>
              </div>
      ...
      <footer className="px-5 py-10 mt-16" style={{ borderTop: '1px solid var(--line-soft)', background: 'var(--bg-2)' }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5 text-[0.82rem]" style={{ color: 'var(--fg-muted)' }}>
            ...
            <a href="https://www.instagram.com/olhausgrupoinmobiliario/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5"><Instagram size={14} /> @olhausgrupoinmobiliario</a>
          </div>
      ...
      {/* LIGHTBOX */}
      {lightbox && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: 'rgba(7,7,9,0.96)' }} onClick={() => setLightbox(false)}>
          ...
          <span className="absolute bottom-6 left-1/2 -translate-x-1/2 text-sm" style={{ color: 'var(--fg-muted)' }}>{active + 1} / {imgs.length}</span>
        </div>
      )}
    </div>
  )
